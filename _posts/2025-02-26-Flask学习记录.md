...
在Flask中，相应即是函数的返回值，可以是字符串、元组、Response对象、字典等。如果返回不是Response对象，Flask会将其隐式地转换为Response对象。
...
Flask 本身是一个轻量级的框架，但是可以通过扩展来增加功能。Flask 的扩展是 Python 包，可以添加应用程序的功能。Flask 本身没有数据库抽象层、表单验证或其它任何已有功能，但是 Flask 的扩展可以添加这些功能。
