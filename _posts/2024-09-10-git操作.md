...
官方文档：[Git Submodule](https://git-scm.com/book/zh/v2/Git-%E5%B7%A5%E5%85%B7-%E5%AD%90%E6%A8%A1%E5%9D%97) 
...
$ git submodule update --remote <submodule-dir> # 更新指定子模块
...
使用以下命令可以指定子模块的分支:
```shell
git config -f .gitmodules submodule.DbConnector.your-Submodule-name brachch-name # 指定子模块分支
```

或者直接在`.gitmodules` 文件中指定子模块的分支:
```shell
[submodule "submodule-name"]
    path = submodule-path
    url = submodule-url
    branch = branch-name # 指定子模块分支
```

之后执行以下命令更新子模块:
```shell
$ git submodule update --remote <submodule-name> # 更新子模块并合并
```
