...
title: 概率论与数理统计
description: 是《概率论与数理统计-张帼奋》的学习笔记
author: [diexie, ] # 此处可以简写，详细添加在 _data/authors.yml 中
date: 2024-09-12 20:00:00 +0800

pin: false # 置顶
categories: [数学] # 分类,第一个为主分类
tags: [数理统计,数学] # 标签
...
1. $\chi^2$分布: 若$X_1, X_2, \cdots, X_n$是n个相互独立的标准正态分布随机变量，即$X_i \sim N(0,1)$，则统计量$Y = \sum_{i=1}^{n}X_i^2$服从自由度为n的$\chi^2$分布，记作$Y \sim \chi^2(n)$。

2. t分布: 若$X \sim N(0,1)$，$Y \sim \chi^2(n)$，则统计量$T = \frac{X}{\sqrt{Y/n}}$服从自由度为n的t分布，记作$T \sim t(n)$。

3. F分布: 若$X \sim \chi^2(n_1)$，$Y \sim \chi^2(n_2)$，则统计量$F = \frac{X/n_1}{Y/n_2}$服从自由度为$(n_1, n_2)$的F分布，记作$F \sim F(n_1, n_2)$。
...
2. 极大似然估计: 极大似然估计是利用样本信息估计总体参数的方法，其核心思想是选择使得样本出现的概率最大的参数作为估计值。
...
1. 置信区间: 置信区间是总体参数的估计区间，其形式为$[\hat{\theta}_1, \hat{\theta}_2]$，其中$\hat{\theta}_1, \hat{\theta}_2$分别是参数的下限和上限估计值。

2. 置信度: 置信度是指总体参数的真实值落在置信区间内的概率，通常用$1-\alpha$表示，其中$\alpha$称为显著性水平。

  $$
  P\{\widehat{\theta}_L<\theta<\widehat{\theta}_U\}\geqslant1-\alpha,
  $$

3. 枢轴量法: 枢轴量是指一个统计量，其分布不依赖于总体参数，通常用来构造置信区间。

  枢轴量法的基本步骤：

  1. 找到一个统计量T，使得T的分布不依赖于总体参数。
  2. 根据样本数据计算T的值，得到T的分布。
  3. 根据T的分布，计算置信区间。
...
1. $\sigma$已知: 正态总体均值的置信区间为：

    $$
    \left[\bar{X}-Z_{\alpha/2}\frac{\sigma}{\sqrt{n}}, \bar{X}+Z_{\alpha/2}\frac{\sigma}{\sqrt{n}}\right]
    $$

2. $\sigma$未知: 正态总体均值的置信区间为：
...
方差$\sigma^2$的区间估计为：(置信度为$1-\alpha$)
